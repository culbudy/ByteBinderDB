...
typedef enum {     // metacommandResults for commands starting with '.'
  META_COMMAND_SUCCESS,
  META_COMMAND_UNRECOGNIZED_COMMAND
} MetaCommandResult;

typedef enum {     // checking for valid commands
  PREPARE_SUCCESS, 
  PREPARE_NEGATIVE_ID,
...
typedef enum {     // what kind of operation to do
  STATEMENT_INSERT, 
  STATEMENT_SELECT,
  STATEMENT_DELETE,
  STATEMENT_UPDATE 
...
typedef struct {   // just for rename statement type
  StatementType type;
  Row row_to_insert;
  int id_to_select;
} Statement;

typedef enum {     // for execution results
  EXECUTE_SUCCESS, 
  EXECUTE_DUPLICATE_KEY,
  EXECUTE_TABLE_FULL,
  EXECUTE_ACCESS_FAILURE
} ExecuteResult;
...
InputBuffer* new_input_buffer() {
  ...
  input_buffer->buffer_length = 1000;
  input_buffer->buffer = malloc(sizeof(char)*1000);
...
void read_input(InputBuffer* input_buffer) {
    ...
    ssize_t bytes_read = strlen(input_buffer->buffer);
...
// do something with meta commands
...
// preparing statements for valid commands
...
// executing operations as per input commands
...
ExecuteResult execute_select(Statement* statement, Table* table) {
    if(statement->id_to_select!=-1){
      uint32_t num_pages = table->pager->num_pages;
      uint32_t key_selected = statement->id_to_select;
      Cursor* cursor = table_find(table, key_selected);
      void* node = get_page(cursor->table->pager,cursor->page_num);
      uint32_t key_at_index = *leaf_node_key(node, cursor->cell_num);

      if(key_at_index==key_selected){
        Row selected_row;
        deserialize_row(cursor_value(cursor),&selected_row);
        print_row(&selected_row);
      }
      else{
        return EXECUTE_ACCESS_FAILURE;
      }
    }
    else{
      Row row;
...
ExecuteResult execute_update(Statement* statement, Table* table) {
    ...
    Cursor* cursor = table_find(table, key_to_insert);
    void* node = get_page(cursor->table->pager, cursor->page_num);
    serialize_row(row_to_insert,leaf_node_value(node,cursor->cell_num));
    return EXECUTE_SUCCESS;
}
